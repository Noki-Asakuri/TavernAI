...
router.get("/characters", function (request, response_characloud_getallcharacters) {
	try {
		client
			.get(charaCloudServer + "/api/characters", function (data, response) {
...
router.get("/board", function (request, response_characloud_getallcharacters) {
	try {
		...
		client
			.get(
				charaCloudServer + `/api/characters/board?nsfw=${nsfw}`,
			...
			.get(url, (response) => {
				...
				file.on("finish", async function () {
					...
					try {
						...
						return response_characloud_loadcard
							.status(400)
...
router.post("/characters/search", jsonParser, function (request, response_characloud_search) {
	...
	try {
		...
		client
			.get(
				charaCloudServer + `/api/characters?q=${encoded_search_string}&nsfw=${nsfw}`,
				function (data, response) {
					if (response.statusCode === 200) {
...
router.post("/server/status", jsonParser, function (request, response_characloud_server_status) {
	try {
		client
			.get(charaCloudServer + "/api/server/status", function (data, response) {
				if (response.statusCode === 200) {
...
router.post(
	"/users/registration",
	jsonParser,
	function (request, response_characloud_registration) {
		try {
			...
			client
				.post(
					charaCloudServer + "/api/users/registration",
					args,
					function (data, response) {
						try {
							if (response.statusCode === 201) {
								...
								console.log(data);
								return response_characloud_registration
...
router.post("/users/login", jsonParser, function (request, response_characloud_login) {
	try {
		...
		client
			.post(charaCloudServer + "/api/users/login", args, function (data, response) {
				try {
					if (response.statusCode === 200) {
...
router.post("/characters/prepublish", urlencodedParser, async function (request, response) {
	try {
		...
		if (fileSizeInKB > 800) {
			...
			return response
				.status(400)
				.json({ error: `The maximum file size should not exceed 800 kb` });
		}
		sharp(sourcePath)
			.metadata()
			.then(async (metadata) => {
				...
				if (!fs.existsSync(sourcePath)) {
					...
					return response
						.status(400)
						.json({ error: `Error: ${sourcePath} does not exist` });
				}
					...
					try {
						...
						if (character_data.name !== undefined) {
							return response.status(200).json({
								character: character_json_data,
								image: `${filename}.${metadata.format}`,
								image_size: fileSizeInKB,
							});
						} else {
							throw new Error("Failed to read character data");
						}
...
router.post(
	"/characters/publish",
	jsonParser,
	async function (request, response_characloud_publish) {
		try {
			...
			if (type === "edit_online") {
				if (
					character_data.user_name !== undefined &&
					character_data.public_id_short !== undefined
				) {
			...
			if (type === "create_online" || type === "edit_online") {
				await charaWrite(
					`./public/cardeditor/${character_img}`,
					character_data_json,
					`./public/cardeditor/${new_file}`,
					"webp",
				);
				...
				needle.post(
					url,
					data,
					{ multipart: true, headers: headers, timeout: 10000 },
					function (err, result) {
						if (err && err.code === "ECONNRESET") {
							console.error("Timeout error:", err);
							// Handle the timeout error here
							return response_characloud_publish
								.status(503)
				...
				await charaWrite(
					`./public/cardeditor/${character_img}`,
					character_data_json,
					`./${charactersPath}${target_filename}`,
					characterFormat,
				);
...
router.post(
	"/user/characters",
	jsonParser,
	function (request, response_characloud_user_characters) {
		try {
			const { nsfw } = request.query;
			...
			client
				.get(
					charaCloudServer +
						`/api/users/${name}/characters?perpage=${perpage}&page=${page}&nsfw=${nsfw}`,
					options,
					function (data, response) {
						try {
							if (response.statusCode === 200) {
								...
								console.log(data);
								return response_characloud_user_characters
									.status(response.statusCode)
									.json(data);
							}
						} catch (err) {
...
router.post("/characters/get", jsonParser, function (request, response_characloud_character) {
	try {
		...
		client
			.get(
				charaCloudServer + `/api/characters/${user_name}/${public_id_short}`,
				function (data, response) {
					try {
						if (response.statusCode === 200) {
								...
								.get(url, (response) => {
									...
									file.on("finish", async function () {
										...
										try {
											...
											const fileSizeInKB = fileSizeInBytes / 1024;
											let char = json5.parse(char_json);
											return response_characloud_character.status(200).json({
												online_data: data,
												character_data: char,
												image: img_card_name,
												image_size: fileSizeInKB,
											...
											return response_characloud_character
												.status(400)
												.json({ error: error });
										}
									});
...
router.post("/characters/delete", jsonParser, function (request, response_characloud_delete) {
	try {
		...
		client
			.delete(
				charaCloudServer + `/api/characters/${user_name}/${public_id_short}?mode=${mode}`,
				options,
				function (data, response) {
					try {
						if (response.statusCode === 200) {
							...
							console.log(data);
							return response_characloud_delete
								.status(response.statusCode)
								.json(data);
						}
					} catch (err) {
...
router.post("/characters/avatar", urlencodedParser, async function (request, response) {
	try {
		...
		const outputPath = path.join(
			...
			"public",
			"cardeditor",
			filedata.filename + ".webp",
		);
		await sharp(sourcePath).resize(400, 600).toFormat("webp").toFile(outputPath);
		...
		sharp(sourcePath)
			...
			.then(async (metadata) => {
				try {
					...
					await sharp(sourcePath).resize(100, 100).toFormat("webp").toFile(destPath);
...
router.post("/category/characters", jsonParser, function (request, response_characloud_category) {
	try {
		...
		client
			.get(
				charaCloudServer + `/api/categories/${category}/characters?nsfw=${nsfw}`,
				function (data, response) {
					try {
						if (response.statusCode === 200) {
							...
							console.log(data);
							return response_characloud_category
								.status(response.statusCode)
								.json(data);
						}
					} catch (err) {
...
router.post("/categories", jsonParser, function (request, response_characloud_category) {
	try {
		let { category } = request.body;
		client
			.get(charaCloudServer + `/api/categories`, function (data, response) {
				try {
					if (response.statusCode === 200) {
						return response_characloud_category.status(200).json(data);
					} else {
...
function generateMainKey(password) {
	ALPHA_KEY = crypto
		.createHash("sha256")
		.update("ALPHA_KEY_" + password)
		.digest("hex");
	BETA_KEY = crypto
		.createHash("sha256")
		.update("BETA_KEY_" + password)
		.digest("hex");
	return crypto
		.createHash("sha256")
