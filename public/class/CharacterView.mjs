...
export class CharacterView extends EventEmitter {
	...
	static SORTERS = {
		...
		DATE_ADD: (a, b) => {
			...
			if (!a.children && !b.children)
				return b.payload.add_date_local - a.payload.add_date_local;
			return a.children ? -1 : 1;
		},
		DATE_ADD_DESC: (a, b) => {
			if (a.children && b.children) return a.name.localeCompare(b.name);
			if (!a.children && !b.children)
				return a.payload.add_date_local - b.payload.add_date_local;
			return a.children ? -1 : 1;
		},
		DATE_ACTION: (a, b) => {
			if (a.children && b.children) return a.name.localeCompare(b.name);
			if (!a.children && !b.children)
				return b.payload.last_action_date - a.payload.last_action_date;
			return a.children ? -1 : 1;
		},
		DATE_ACTION_DESC: (a, b) => {
			if (a.children && b.children) return a.name.localeCompare(b.name);
			if (!a.children && !b.children)
				return a.payload.last_action_date - b.payload.last_action_date;
			return a.children ? -1 : 1;
		},
	};
				...
				options.input.sortSelect.onchange = function (event) {
					if (CharacterView.SORTERS[event.target.value.toUpperCase()]) {
						this.controller.sort(
							CharacterView.SORTERS[event.target.value.toUpperCase()],
						);
					}
				}.bind(this);
			}
		...
		if (event.dataTransfer.items) {
			...
			if (filtered.length) {
				this.emit(CharacterView.EVENT_FILES_IMPORT, {
					...
					type: CharacterView.EVENT_FILES_TYPE.DROP,
				});
			}
		}
	}
	...
	onFilesFromForm(event) {
		...
		this.emit(CharacterView.EVENT_FILES_IMPORT, {
			files: filtered,
			type: CharacterView.EVENT_FILES_TYPE.FORM,
		});
	}
