...
export class UIFolder extends UIFolderContent {
	...
	onDrop(event) {
		...
		let target = this;
		if (
			UIFolderContent.dragged.parent &&
			UIFolderContent.dragged.parent === this &&
			this.parent
		) {
			target = this.parent;
		}
		...
		do {
			...
			this.children.every((child) => {
				if (
					...
					child.name.toLowerCase() === candidate.toLowerCase() + suffix
				) {
					match = true;
					return false;
				}
	...
	filter(rules, root = true) {
		...
		this.children.forEach((child) => {
			if (child.folder) {
				valid = valid.concat(child.filter(rules, false));
			} else {
	...
	delete(recursive, noEmit = false) {
		...
		if (!noEmit) {
			this.emit(UIFolderContent.EVENT_DELETED, {
				parent: oldParent,
				propagate: true,
				target: this,
			});
