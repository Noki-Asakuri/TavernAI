...
export class WPPEditor extends EventEmitter {
		...
		for (let index = 0; index < this.editor.childNodes.length; index++) {
			for (let i = 0; i < this.editor.childNodes[index].length; i++) {
				...
				if (this.editor.childNodes[index][i].classList.contains('props')) {
					for (let j = 0; j < this.editor.childNodes[index][i].childNodes.length; j++) {
						// rows
						for (
							let k = 0;
							k < this.editor.childNodes[index][i].childNodes[j].childNodes.length;
							k++
						) {
			...
			for (let key in this._wpp[event.target.propertyIndex].properties) {
				if (key === event.target.propertyName) {
					newProps[event.target.value] =
						this._wpp[event.target.propertyIndex].properties[key].slice();
				} else {
	...
	updatePropertyValue(event) {
		...
		this._wpp[event.target.propertyIndex].properties[event.target.propertyName][
			event.target.valueIndex
		] = event.target.value;
		...
		if ((!event.target.value || !event.target.value.length) && event.target.nextSibling) {
			...
			this._wpp[event.target.propertyIndex].properties[event.target.propertyName].splice(
				event.target.valueIndex,
				1,
			);
