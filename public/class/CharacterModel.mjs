...
export class CharacterModel extends EventEmitter {
	...
	onFilesImport(event) {
		...
		this.importCharacters(event.files).then(
			(resolve) => {
	...
	importCharacters(files, strict = false, successes = [], failures = []) {
		return new Promise((resolve, reject) => {
			this.importCharacter(files.shift()).then(
				(success) => {
					successes.push(success);
					if (files.length) {
						this.importCharacters(files, strict, successes, failures).then(
							resolve,
							reject,
						);
					} else {
