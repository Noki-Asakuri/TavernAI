...
export class CharacterEditor extends EventEmitter {
	...
	constructor(options) {
		...
		this.other.roomCharacterSelect = this.findChildWithClass(
			"room-character-select-div",
			this.container,
			true,
		);
		this.other.roomSelectedCharacters = this.findChildWithClass(
			"room-character-selected-items",
			this.container,
			true,
		);
		this.other.roomAvailableCharactersDiv = this.findChildWithClass(
			"room-character-select-items-div",
			this.container,
			true,
		);
		this.other.roomScenarioInput = this.findChildWithClass(
			"room-scenario",
			this.container,
			true,
		);
	...
	get tokenCountMax() {
		...
		let scenario = getIsRoom() ? data.room_scenario : data.scenario;
		return encode(
			JSON.stringify(data.description + data.personality + scenario + data.mes_example),
		).length;
	}
	...
	refresh(useImageUrl) {
		...
		if (getIsRoom() && this._editMode) {
			this.other.roomScenarioInput.value =
				getRoomsInstance().id[getRoomsInstance().selectedRoomId].chat[0].scenario || "";
		}
