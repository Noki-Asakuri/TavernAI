	...
	updateWPPProperty(index, event) {
		...
		if (!event.target.value || !event.target.value.length) {
			if (event.target.nextSibling) {
				...
				let i = index;
				while (next) {
					next.onchange = setTimeout.bind(this, this.updateWPPProperty.bind(this, i), 0);
	...
	updateKey(index, minor, event) {
		...
		if (!event.target.value || !event.target.value.length) {
			if (event.target.nextSibling) {
				...
				let i = index;
				while (next) {
					next.onchange = setTimeout.bind(this, this.updateKey.bind(this, i, minor), 0);
					next.onkeyup = next.onchange;
					next.oncut = next.onchange;
					next.onpaste = next.onchange;
					next = next.nextSibling;
