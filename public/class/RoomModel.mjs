...
export class RoomModel extends EventEmitter {
	...
	setActiveCharacterId(chatHistory) {
		...
		for (var i = chatHistory.length - 1; i >= 0; i--) {
			if (!chatHistory[i]["is_user"]) {
				// We want to get the latest message by a character, not user (user's chid == -1)
				this.activeCharacterIdInit(chatHistory[i]);
				found = true;
				break;
			}
		}
		if (!found) this.activeCharacterIdInit();
	}
	...
	setNextActiveCharacter() {
		...
		else this.activeId = this.selectedIDs[this.activeCharacterIndex + 1];
	...
	onRoomShown() {
		...
		let characterContainer = findChildWithClass(
			"room-character-select-items",
			this.characters.editor.other.roomCharacterSelect,
			true,
		);
	...
	onRoomDelete(event) {
		...
		jQuery.ajax({
			...
			success: function (html) {
				this.rooms = this.rooms.filter((room) => room.filename != event.target + ".jsonl");
