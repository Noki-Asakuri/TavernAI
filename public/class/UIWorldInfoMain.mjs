...
export class UIWorldInfoMain extends Resizable {
    ...
    */
	...
	refresh() {
		...
		this.entriesArray = arr.sort(
			(a, b) =>
				(a.order !== undefined ? a.order : 9999) - (b.order !== undefined ? b.order : 9999),
		);
		let max = 0;
		arr.forEach((v, i) => {
			v.order = i;
	...
	onDragOver(options, event) {
		...
		for (let key in row.parentNode.children) {
			if (row.parentNode.children[key] === row) {
				toIndex = i;
			}
			if (row.parentNode.children[key] === this.dragged) {
				fromIndex = i;
			}
			i++;
		}
	...
	onMoveItem(options, event) {
		...
		let index = 0;
		let sorted = this.entriesArray.sort(
			(a, b) =>
				(a.order !== undefined ? a.order : 9999) - (b.order !== undefined ? b.order : 9999),
		);
	...
	onDeleteItem(options, event) {
		...
		if (order !== undefined && order !== null) {
			...
			if (!shared) {
				for (let uid in this.data.entries) {
					this.data.entries[uid].order =
						this.data.entries[uid].order >= order
							? this.data.entries[uid].order - 1
							: this.data.entries[uid].order;
				}
			}
		}
			...
			for (let i = 0; i < sorted.length; i++) {
				...
				if (included) {
					if (entry.selective) {
						...
						if (inPrimary >= 0) {
							(entry.prepend ? prepend : append).push(
								hasPrimary.splice(inPrimary, 1)[0].content,
							);
							(entry.prepend ? append : prepend).push(null);
							sorted.splice(i, 1);
							i--;
